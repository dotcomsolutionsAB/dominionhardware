...
class AdminController extends Controller
{
    ...
    {
        // Fetch google_sheet data and join with users table to get the seller name
        $data = DB::table('google_sheet')
            ->join('users', 'google_sheet.seller_id', '=', 'users.id')
            ->select('google_sheet.*', 'users.name as seller_name','users.user_type as seller_type')
            ->get();

        // Fetch only sellers with id 9 or 91 from users table
        $sellers = DB::table('users')
            ->select('id', 'name','user_type')
            ->whereIn('id', [9, 91])
            ->get();

        // Pass both data and sellers to the view
        return view('backend.sync', compact('data', 'sellers'));
    }

    // Method to sync the status
    public function syncStatus($id)
    {
        try {
            // Update status to 0 and update last_update with the current timestamp
            DB::table('google_sheet')->where('id', $id)->update([
                'status' => 0,
                'last_update' => Carbon::now() // Use Carbon to set the current timestamp
            ]);

            // Return success response with last_update data
            return response()->json([
                'success' => true,
                'last_update' => Carbon::now()->toDateTimeString() // Send the current timestamp to the frontend
            ]);
        } catch (\Exception $e) {
    ...
    // Method to check the status for polling
    public function checkStatus($id)
    {
        try {
            // Get both status and last_update from the google_sheet table
            $data = DB::table('google_sheet')->select('status', 'last_update')->where('id', $id)->first();

            // Return status and last_update as JSON
            return response()->json([
                'status' => $data->status,
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'sheet_path' => 'required|string|max:255',
        ]);

        // Insert the new Google Sheet record into the database
        DB::table('google_sheet')->insert([
        ...
        return redirect()->back()->with('success', 'Google Sheet added successfully.');
    }
    ...
    public function admin_dashboard(Request $request)
    {
        ...
        // $recentCarts = Cart::orderBy('created_at', 'desc')->limit(10)->get();
        // return view('backend.view_cart', compact('carts'));
